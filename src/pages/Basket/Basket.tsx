    ...
    return (
        <main className={styles.page}>
            <div className={gStyles.container}>
                ...
                <SwitchContent disabled={[0, 1, 2]} links={[{ name: SWITCH_CONTENT[0], to: `${PATH_BASKET}/${SWITCH_CONTENT[0]}` }, { name: SWITCH_CONTENT[1], to: `${PATH_BASKET}/${SWITCH_CONTENT[1]}` }, { name: SWITCH_CONTENT[2], to: `${PATH_BASKET}/${SWITCH_CONTENT[2]}` }]} />
            </div>
            <Routes>
                ...
                <Route path={SWITCH_CONTENT[2]} element={<Confirmation payment={state.payment} address={{ street: field.street, house: field.house }} deliveryMethod={state.deliveryMethod} totalPrice={state.totalPrice} products={state.selectedProducts} />} />
            </Routes>
        </main>
    );
}