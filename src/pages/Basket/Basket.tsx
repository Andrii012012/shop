...
export default function Basket(): JSX.Element {
    ...
    function checkElementForRecipe(): boolean {
        const element = state.selectedProducts.find((item) => item.isRecipe && item);
    ...
    return (
        <main className={styles.page}>
            <div className={gStyles.container}>
                ...
                <SwitchContent
                    disabled={[{ index: 0, color: "" }, { index: 1, color: isRecipeProduct ? "" : "#7F7F7F" },
                    { index: 2, color: '' }]}
                    links={[
                        { name: BASKET_SECTIONS.myBasket, to: `${PATH_BASKET}/${BASKET_SECTIONS.myBasket}` },
                        { name: BASKET_SECTIONS.methodDelivery, to: `${PATH_BASKET}/${BASKET_SECTIONS.methodDelivery}` },
                        { name: BASKET_SECTIONS.paymentConfirmation, to: `${PATH_BASKET}/${BASKET_SECTIONS.paymentConfirmation}` }
                    ]}
                />
            </div>
            <Routes>
